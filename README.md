...
- **AI 对话教练**：说话即可记录训练、管理食物库、生成计划、手动记录运动消耗；自动注入今日/本周数据与历史摘要精准回答，必要时联网查证；function calling 只写不删
- **运动与热量**：GPS 跑步 / 骑行 / 步行（前台服务）、系统步数统计、运动统计独立 Tab、热量缺口评估、AI 每日复盘
- **看板与小组件**：消耗 / 摄入 / 反向三块数据、热量进度与差额、体重折线、AI 每日复盘、一键备份 / 恢复、Glance 桌面小组件
