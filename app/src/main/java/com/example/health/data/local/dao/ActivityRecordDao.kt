...
interface ActivityRecordDao {
    ...
    @Query("SELECT * FROM activity_record WHERE date(startTime / 1000, 'unixepoch') = :date ORDER BY startTime DESC")
    suspend fun getRecordsByDate(date: String): List<ActivityRecord>

    @Query("SELECT * FROM activity_record WHERE date(startTime / 1000, 'unixepoch') BETWEEN :startDate AND :endDate ORDER BY startTime DESC")
    suspend fun getRecordsBetweenDates(startDate: String, endDate: String): List<ActivityRecord>

    @Query("SELECT COALESCE(SUM(caloriesKcal), 0) FROM activity_record WHERE date(startTime / 1000, 'unixepoch') = :date")
    suspend fun getTotalCaloriesByDate(date: String): Int

    @Query("SELECT COALESCE(SUM(caloriesKcal), 0) FROM activity_record WHERE date(startTime / 1000, 'unixepoch') BETWEEN :startDate AND :endDate")
